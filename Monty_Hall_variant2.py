...
# Now 2 doors contain prize, have k doors and Zonk opens s doors
...
def pick_door(k, s):
    doors = ['goat'] * (k - 2) + ['car'] * 2  # set k doors, two contain prize
...
for s in zonk_open:
    for k in door_num:
        # if Zonk picked doors are one less than or equal to the existing doors, then this game doesn't work
        if k <= s + 1:
            stay_prob = 'N/A'
            switch_prob = 'N/A'
